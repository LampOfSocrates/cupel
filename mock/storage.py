...
SQLite is the database in BOTH cases:
  ...
  CUPEL_STORAGE=local  (default)  a plain SQLite file on the machine running
                                  it. A developer checkout gets exactly this,
                                  with no S3 code path and no extra env.
  CUPEL_STORAGE=s3                the same SQLite file, continuously
...
# Key prefix inside the bucket when CUPEL_S3_PATH is not set. Mirrored by
# scripts/dev.mjs (banner) — keep the two in step.
DEFAULT_S3_PATH = "cupel-mock"
...
def mode(env=None) -> str:
    ...
    replication"."""
    raw = (_env(env).get("CUPEL_STORAGE") or "").strip().lower()
...
def health_storage(env=None) -> dict:
    ...
    CUPEL_STORAGE to "local" in the child env when an s3 boot had to degrade,
    ...
    if active == S3:
        out["restored"] = env.get("CUPEL_STORAGE_RESTORED") == "1"
    return out


def s3_settings(env=None) -> tuple[dict, list[str]]:
    """(settings, missing) from the CUPEL_S3_* env. `missing` names the
    ...
    settings = {
        "bucket": (env.get("CUPEL_S3_BUCKET") or "").strip(),
        "endpoint": (env.get("CUPEL_S3_ENDPOINT") or "").strip(),
        "path": (env.get("CUPEL_S3_PATH") or "").strip() or DEFAULT_S3_PATH,
        "region": (env.get("CUPEL_S3_REGION") or "").strip() or DEFAULT_S3_REGION,
        "access_key_id": (env.get("CUPEL_S3_ACCESS_KEY_ID") or "").strip(),
        "secret_access_key": (env.get("CUPEL_S3_SECRET_ACCESS_KEY") or "").strip(),
    }
...
def litestream_yml(db_path: str, settings: dict) -> str:
    ...
    return "\n".join(
        [
            "# GENERATED by mock/storage.py from the CUPEL_S3_* environment",
            "# variables. Rewritten on every boot — do not edit by hand.",
