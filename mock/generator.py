"""Loom data generator (P1-T18): deterministic seed + basic drip.

Invariant (loom-phases.md:160, feature-spec.md:185): the generator "writes
...
Modes (feature-spec.md:187-188):
  ...
  drip     continuous loop: "every N seconds starts a new conversation, adds
           ...
           the /admin/generator control API are Phase 2, loom-phases.md:98).
  simulate seed then drip — what `npm run simulate` runs (loom-phases.md:43).
...
Idempotency of re-running seed with the same --seed:
  ...
  - judging/thumbs check GET /eval/judgments first and skip when present.
    Judgments are append-only BY DESIGN (loom-phases.md:160), so if a judge
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        ...
        description="Fill a RUNNING Loom mock with realistic data, writing only "
