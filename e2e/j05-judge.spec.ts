...
test("judge: run with the judge on → scores stream in → drawer reasoning + append-only history", async ({
  ...
  await test.step("configure a run with the judge enabled up front", async () => {
  ...
  await test.step("queue: cells and SCORES both stream into the grid", async () => {
    api.clear();
  ...
  await test.step("judgment drawer: reasoning and append-only history", async () => {
    api.clear();
  ...
  await test.step('retroactive "Judge this run" appends, never overwrites', async () => {
    api.clear();
