...
- Free tier constraints, accepted:
  ...
  - No persistent disk → the container's filesystem is ephemeral. See
    **Storage modes** below: `SKEIN_STORAGE=local` accepts that (every restart
    starts from the seed), `SKEIN_STORAGE=s3` replicates the SQLite file to a
...
| `SKEIN_STORAGE` | What it is | Who runs it |
| --- | --- | --- |
| `local` (default) | A plain SQLite file at `SKEIN_MOCK_DB`. No S3 code path, no extra env, no extra process. | Every developer checkout. Also the current, working Render default. |
...
1. renders `/etc/litestream.yml` from the `SKEIN_S3_*` env
...
- never point a second service, a second environment, or a local `npm start`
  at the same `SKEIN_S3_BUCKET` + `SKEIN_S3_PATH`. Use a different prefix for
  anything else;
...
1. Cloudflare dashboard → **R2** → *Create bucket* → name it `skein-demo`,
...
4. Render dashboard → the `skein-demo` service → **Environment** → add:

   | Variable | Value | Required |
   | --- | --- | --- |
   | `SKEIN_STORAGE` | `s3` | yes |
   | `SKEIN_S3_BUCKET` | `skein-demo` | yes |
   | `SKEIN_S3_ENDPOINT` | `https://<account-id>.r2.cloudflarestorage.com` | yes |
   | `SKEIN_S3_ACCESS_KEY_ID` | R2 token access key id | yes |
   | `SKEIN_S3_SECRET_ACCESS_KEY` | R2 token secret | yes |
   | `SKEIN_S3_PATH` | key prefix inside the bucket (default `skein-mock`) | no |
   | `SKEIN_S3_REGION` | `auto` for R2; the real region for AWS | no |

   Keep `SKEIN_SEED_ON_BOOT=1` and `SKEIN_SEED=42` — they now mean *seed if
...
For **AWS S3** instead: same variables, `SKEIN_S3_ENDPOINT` =
`https://s3.<region>.amazonaws.com`, `SKEIN_S3_REGION` = the real region, and
an IAM user with `s3:GetObject/PutObject/DeleteObject/ListBucket` on that
bucket only.

### Seed on boot = seed **if empty**
`SKEIN_SEED_ON_BOOT=1` used to re-seed on *every* boot — the mitigation for a
...
| A `SKEIN_S3_*` variable is missing | Logged with the exact variable names, and the container runs **local and unreplicated**. `/healthz` reports `mode:"local"`, so it never claims durability it does not have. |
...
`SKEIN_STORAGE=s3` plus the `SKEIN_S3_*` variables makes `npm start` boot
...
`SKEIN_S3_PATH` than the deployed demo** — see the single-writer rule.
