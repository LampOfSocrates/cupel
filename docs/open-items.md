...
| id | item | status |
|---|---|---|
| PH-2 | **Done 2026-08-10.** Was: Render hostname was `skein.onrender.com`, not `cupel-demo.onrender.com` — two separate services existed (a Docker demo at `skein.onrender.com` and a standalone static-site landing page at `cupel-site.onrender.com`). Resolved by merging to ONE origin: `mock/root.py` serves the landing page at `/` and mounts the whole demo at `/cupel-demo` (Starlette `Mount`, zero prefix threaded through `mock/main.py`'s routes — `/cupel-demo/openapi.json` works for free via `root_path`); the `DEMO_TOKEN` shared-token gate is deleted entirely (Phase-1 demo data isn't sensitive). Both old services were deleted (not just suspended — suspension keeps the slug reserved and blocks recreating under the same name, learned the hard way) and a fresh Docker service named `cupel-site` created via the Render API from `render.yaml`'s config, which reclaimed the `cupel-site.onrender.com` hostname. One deploy failure en route: the Dockerfile's new `COPY docs/index.html`/`COPY docs/assets` had `.dockerignore` blanket-excluding `docs/`, so the build failed with nothing to copy — fixed by narrowing the ignore, not the COPY (`.dockerignore` only gates the build context, not the final image). **Verified live**: `/`, `/assets/cupel-photo.jpg`, `/cupel-demo/healthz`, `/cupel-demo/openapi.json`, `/cupel-demo/` all correct at `https://cupel-site.onrender.com/`. | done |
