...
| id | item | status |
|---|---|---|
| PH-2 | **Code done 2026-08-10, dashboard step still pending (user).** Was: Render hostname is `skein.onrender.com`, not `cupel-demo.onrender.com` — two separate services existed (a Docker demo at `skein.onrender.com` and a standalone static-site landing page at `cupel-site.onrender.com`). Resolved in code by merging to ONE origin: `mock/root.py` serves the landing page at `/` and mounts the whole demo at `/cupel-demo` (Starlette `Mount`, zero prefix threaded through `mock/main.py`'s routes — `/cupel-demo/openapi.json` works for free via `root_path`); `render.yaml`'s service renamed `cupel-demo` → `cupel-site` to target the surviving hostname; the `DEMO_TOKEN` shared-token gate is deleted entirely (Phase-1 demo data isn't sensitive). **Still needs a human in the Render dashboard**: delete the `skein.onrender.com` Docker service and the standalone `cupel-site` static site, then apply the (renamed) blueprint fresh — see docs/deployment.md "Consolidating the two existing Render services into this one". | dashboard-only, user |
