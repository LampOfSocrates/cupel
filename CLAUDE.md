...
Current phase: 2. Current task: P2-RECORD Minimal e2e recording rig.
...
- E2e: Playwright (e2e/, `npm run e2e` = 13 journeys × both auth modes; `e2e:smoke` / `e2e:auth` are subsets) — opt-in, NOT part of `npm test`; boots the real mock + vite via webServer on a scratch SQLite (SKEIN_MOCK_DB), never the dev DB, seeded from the deterministic generator (seed 42). One spec per journey (`e2e/jNN-*.spec.ts`), each asserting the sketch's endpoint tags via `e2e/helpers/api.ts`. See README "End-to-end suite".
