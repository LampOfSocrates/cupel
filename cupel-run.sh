#!/usr/bin/env bash
# cupel-run.sh — one self-contained task runner for Cupel. (Git Bash / macOS / Linux)
...
#
# Usage: ./cupel-run.sh [max_tasks]     DRY=1 ./cupel-run.sh  (preview only)
...
for f in TASKS.md feature-spec.md cupel-phases.md react-migration.md CLAUDE.md; do
...
for i in $(seq 1 "$MAX"); do
  ...
  PROMPT="Read CLAUDE.md, react-migration.md, cupel-phases.md, and feature-spec.md.
...
7. Stop. Do not start the next task."
  ...
  if [ -n "$FAIL" ]; then
    ...
    echo "Stopping for review. Fix, then re-run ./cupel-run.sh"
    exit 1
  fi
