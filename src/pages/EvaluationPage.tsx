...
// (debounced ~300ms) on task/progress events of this evaluation's task family, until
// the evaluation status is terminal (refetch-on-event is the documented baseline;
...
// - summary header from GET /eval/evaluations/{evaluationId}/summary (openapi.yaml:1001-1022)
// - score chips via the grid's renderAnnotation slot on judged cells
//   (Result.case_id + latest_score, openapi.yaml:1654-1664); tap → drawer
// - judgment frames on /tasks/stream (openapi.yaml:1796-1804) refetch run +
//   summary on the same debounce
// - the stream stays subscribed past evaluation-terminal WHILE a judging task is in
//   flight; judgments fired elsewhere (e.g. curl) appear on reload only.
//
// Cancel = DELETE /tasks/{task_id} on the evaluation's parent task (openapi.yaml:
...
// A judging task still owns the evaluation's scores while it is queued or running.
...
// are fetched once per mount; everything below them belongs to one (tree, evaluationId)
...
// … }, [tree, evaluationId])` was hand-rolling — and unlike that effect the key also
...
function EvaluationBody({ rubrics }: { rubrics: Rubric[] }) {
  ...
  const fireJudge = useCallback(
    ...
    // — a background judge that 422s ("Evaluation has no finished cells to judge
    ...
    async (judge_model: string, rubric_id: string, silent = false) => {
      try {
        // POST /eval/judge {evaluation_id, judge_model, rubric_id} → 202 "Judging
        // enqueued (parent task + child per case)" (openapi.yaml:931-954);
        // evaluation_id "auto-creating cases from conversation turns if none exist"
        // (feature-spec.md:61).
        const ref = await api.judge({ evaluation_id: evaluationId, judge_model, rubric_id });
  ...
  // NOT "judge an evaluation I happened to watch finish". The old trigger keyed off a
  // live non-terminal → done transition, so opening an evaluation link after the evaluation
  // had already finished silently produced no scores at all.
  //
  // AUTO-JUDGE PREDICATE — all five must hold:
  //   1. run.id === evaluationId && run.status === "done" — the aggregate replay
  ...
  //      an evaluation whose cells ALL failed answers 422 — swallowed above.
  ...
  const autoJudge =
    evaluation?.id === evaluationId && evaluation.status === "done"
      ? (evaluation.columns.map((c) => c.config.judge).find((j): j is JudgeConfig => j != null) ?? null)
      : null;
      ...
      try {
        ...
        const inFlight = (await api.tasks({ limit: 50 })).find(
          (t) =>
            ...
            t.result?.evaluation_id === evaluationId,
        );
  ...
  }, [evaluationId, autoJudgeModel, autoRubricId, fireJudge]);

  const taskId = evaluation?.task_id;
  const terminal = evaluation != null && TERMINAL.has(evaluation.status);
  const judging = judgeTaskId != null;

  // Live fill subscription. Family membership: `task` frames are Task objects
  // — the parent itself (id) or its children (parent_id), for the evaluation's task
  ...
  // frames join by judgment.evaluation_id (openapi.yaml:1800-1802). Subscribed while
  // the evaluation is live OR judging is in flight; terminal + idle → cleanup aborts.
  useEffect(() => {
      ...
      while (!stopped && !controller.signal.aborted) {
        try {
          for await (const ev of api.taskStream({
            ...
            // so an evaluation that finished in that gap would otherwise never
            ...
            if (ev.event === "judgment") {
              belongs = ev.data.judgment.evaluation_id === evaluationId;
  ...
  }, [taskId, terminal, judging, judgeTaskId, evaluationId, reload]);

  const cancel = async () => {
    if (!evaluation) return;
    setCancelling(true);
    try {
      await api.cancelTask(evaluation.task_id);
      reload();
    } catch (e) {
      ...
      <Group justify="space-between" mt={2} wrap="nowrap">
        ...
        <Group gap={2} wrap="nowrap">
          {/* ⌁ — "⌁ trace icon on every turn — in Chat, results
              ...
              the produced turn's id (Result.turn_id, openapi.yaml:1652)
    ...
    <Stack gap="sm" p="md">
      <Group justify="space-between">
        <Group gap="xs">
          ...
          {stage != null && !terminal && (
            <Text size="xs" c="dimmed" data-testid="evaluation-stage">
              {stage}
            </Text>
          )}
        </Group>
        <Group gap="xs">
          ...
          {evaluation.status === "done" && !judging && (
            <Button
              size="compact-xs"
              variant="light"
      ...
      {/* Cell ⑂ via the cell-action slot (separate from renderAnnotation,
          ...
          (Evaluation.rows[].source, openapi.yaml:1607-1643). Sketch 04: "+ Re-run
          this turn with… POST …/replay/turn".

          Fork pivot: for turn re-fire evaluations the server already delivers
          ...
          (Result.conversation_id, openapi.yaml:1651) and the baseline cell
